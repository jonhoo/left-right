...
struct ValueRegistry {
    num_live_values: RefCell<i64>,
}

impl ValueRegistry {
    fn new() -> Self {
        Self {
            num_live_values: RefCell::new(0),
        }
    }

    fn adjust_count(&self, delta: i64) {
        let mut live_vals = self.num_live_values.borrow_mut();
        *live_vals += delta;
        assert!(*live_vals >= 0);
    }

    fn expect(&self, expected_count: i64) {
        assert_eq!(*self.num_live_values.borrow(), expected_count);
    }
}
...
impl Absorb<Op> for Deque {
    ...
    fn drop_second(self: Box<Self>) {
        // Comnvert self to DoDrop and drop it.
